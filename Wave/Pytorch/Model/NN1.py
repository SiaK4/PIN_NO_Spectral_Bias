...
def get_activation(activation_name='swish'):
...
class NN(nn.Module):
    def __init__(self, in_c, out_c, features = [100,100,100,100], activation_name='tanh', num_frequencies=4,\
        fourier_type='random',fourier_scale=1.0, use_siren=False, siren_w0=30.0, skip_con = False, h_siren=False, learning_w0=False):
        ...
        if use_siren:
            ...
            self.layers.append(layer(fourier_out_dim, features[0], self.activation))
            for i in range(1,len(features)-1):
                self.layers.append(layer(features[i],features[i+1], self.activation))
...
class NN_Periodic(NN):
    def __init__(self, in_c, out_c, features = [100,100,100,100], activation_name='tanh', num_frequencies=4,\
        fourier_type='random',fourier_scale=1.0, use_siren=False, siren_w0=30.0, x_left = -1.0, x_right=1.0, skip_con = True):

        super().__init__(in_c, out_c, features, activation_name,\
            num_frequencies, fourier_type, fourier_scale, use_siren, siren_w0)
