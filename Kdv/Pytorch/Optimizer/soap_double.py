            ...
            if merge_dims:
                ...
                for idx, sh in enumerate(new_grad.shape):
                    if sh <= max_precond_dim:
                        ...
                        outer_product = outer_product.to(state['GG'][idx].dtype).to(state['GG'][idx].device)
    ...
    def get_orthogonal_matrix(self, mat):
        ...
        for m in mat:
            ...
            if m.data.dtype != torch.float:
                ...
                matrix.append(m.data.to(torch.float64))
            else:
                float_data = True
                matrix.append(m.data.to(torch.float64))
        ...
        for m in matrix:
            ...
            try:
                ...
                _, Q = torch.linalg.eigh(m.to(torch.float64)+1e-30*torch.eye(m.shape[0], device=m.device, dtype=m.dtype))
                Q = Q.to(m.dtype)
            Q = torch.flip(Q, [1])
    ...
    def get_orthogonal_matrix_QR(self, state, max_precond_dim=10000, merge_dims=False):
        ...
        for m,o in zip(precond_list, orth_list):
            ...
            if m.data.dtype != torch.float:
                ...
                original_device = m.data.device
                matrix.append(m.data.to(torch.float64))
                orth_matrix.append(o.data.to(torch.float64))
            else:
                float_data = True
                matrix.append(m.data.to(torch.float64))
                orth_matrix.append(o.data.to(torch.float64))
