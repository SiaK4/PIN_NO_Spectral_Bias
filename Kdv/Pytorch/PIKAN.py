...
def train(model, epoch_number, learning_rate, domain_data, init_data, epoch_resample,L1_epoch, double=False):
    ...
    scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=2000, gamma=0.95)
...
params = init_params(layers = layers, degree=model_cfg["degree"], initialization_type='normal') 

model = KAN(params = params, x_left=model_cfg["x_left"], x_right=model_cfg["x_right"], degree=model_cfg["degree"])
model.to(device)
