...
class NN_Periodic(NN):    
    ...
    def __call__(self, x):
        ...
        if x.ndim == 1:
            x = x[None, :]  # Add batch dimension: (1, features)

        x_spatial = x[:,0:1]
        # x_spatial = x[0:1]
        phase = 2*jnp.pi * (x_spatial - self.x_left)/period
        x_periodic = jnp.concatenate([jnp.sin(phase), jnp.cos(phase)], axis=-1)
        ...
        ## Fourier layer if needed
        
        ## Use the based class NN's logic to build the MLP
        return super().__call__(x_input)
