...
class LinearLayer(eqx.Module):
    ...
    def __init__(self, key, in_dim, out_dim):
        self.W = jax.random.normal(key, (in_dim, out_dim)) / jnp.sqrt(in_dim)
...
class ChebyKANLayer(eqx.Module):
    ...
    def __call__(self, x):
        # Nonlinearity FIRST (maps to [-1,1])
        x = self.activation(x)

        # Optional safety clip (numerical robustness)
        x = jnp.clip(x, -1.0, 1.0)

        # Chebyshev expansion
        T = chebyshev_stack(x, self.degree)

        # Linear combination
        return jnp.einsum("...id,iod->...o", T, self.W) + self.b
...
class Helmholtz2D(eqx.Module):
    ...
    def __init__(self, key, width=64, degree=5):
        ...
        self.out = LinearLayer(keys[4], width, 1)
...
#     # print(f"PINN Simulation for 2D Helmholtz (Periodic via Fourier features) a1={a1}, a2={a2}, k={K}")
...
pinn = Helmholtz2D(init_key, width=32, degree=7)         # input now 4 due to embedding
...
# plt.title(f"Exact solution\nu(x,y) = sin({a1}πx) sin({a2}πy)", fontsize=14)
plt.xlabel("x"); plt.ylabel("y")
plt.axis("equal")
...
# ============================================================
# === DROP-IN EVALUATION & PLOTTING (Reaction–Diffusion) ====
# ============================================================
