...
class Helmholtz2D(eqx.Module):
    ...
    def __init__(self, key, units=20):
        ...
        self.layers = [
            ...
            eqx.nn.Linear(units, 1, key=key_list[8])
        ]
...
pinn = Helmholtz2D(init_key, units=44)          # input now 4 due to embedding
...
t_end = time.time()
print(f"Quasi-Newton finished in {t_end - t_start:.1f} seconds")
...
def u_pred_fn(x, y):  return pinn(x, y)[0]
...
print(f"p = 0  (L2)        : {E_L2:.6e}")
print(f"p = 2  (Gradient)  : {E_grad:.6e}")
print(f"p = 4  (Laplacian) : {E_lap:.6e}")
print("="*70)
