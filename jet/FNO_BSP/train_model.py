...
model = TFNO(
    n_modes=n_modes,              # Fourier modes in (kx, ky)
    hidden_channels=hidden_channels, # 128 in your Par
    in_channels=in_channels,
    out_channels=out_channels,
    # Optional, but matches the quickstart in the docs:
    factorization='tucker',
    implementation='factorized',
    rank=0.05,                    # ~5% of dense FNO params 
).to(device).to(DTYPE)
